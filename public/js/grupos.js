document.addEventListener("DOMContentLoaded", () => {
    ...
    // Para botones con clase .edit-btn
    ...
    // Función para actualizar horarios por profesor
    ...
    // Seleccionar salón desde modal
    window.seleccionarSalon = function (idMateria, idSalon, textoSalon) {
      const input = document.getElementById(`inputSalon_${idMateria}`);
      const texto = document.getElementById(`salonSeleccionado_${idMateria}`);
  
      if (input && texto) {
        input.value = idSalon;
        texto.innerText = textoSalon;
        texto.classList.remove("text-muted");
        texto.classList.add("text-success", "fw-semibold");
      }
  
      // Fallback de respaldo
      window.ultimoSalonSeleccionado = window.ultimoSalonSeleccionado || {};
      window.ultimoSalonSeleccionado[idMateria] = idSalon;
    };
  
    // Guardar salón al dar clic en "Guardar" en el modal
    window.guardarSalonSeleccionado = function (idMateria) {
      const input = document.getElementById(`inputSalon_${idMateria}`);
      const idSalon = input?.value || window.ultimoSalonSeleccionado?.[idMateria];
      const idCiclo = document.getElementById("cicloActualId")?.value || "";
      ...
      fetch("/grupos/asignar-salon", {
        ...
        .then(data => {
          ...
          const texto = document.getElementById(`salonSeleccionado_${idMateria}`);
          if (texto) {
            texto.innerText = `Asignado: ${idSalon}`;
            texto.classList.remove("text-muted");
            texto.classList.add("text-success", "fw-semibold");
          }
        })
        .catch(error => {
          ...
          alert("Ocurrió un error al asignar el salón");
        });
    };
  });  